...
import cv2
#from eva-decord import VideoReader, cpu
...
def load_video(video_items,input_size=448, max_num=1):
    video, _ = my_custom_read_video_torchvision(video_items)
    transform = build_transform(input_size=input_size)
    pixel_values_list, num_patches_list = [], []
    for frame in video:
        img = T.ToPILImage()(frame).convert("RGB")
    ...
    pixel_values = torch.cat(pixel_values_list) if pixel_values_list else torch.empty(0)
