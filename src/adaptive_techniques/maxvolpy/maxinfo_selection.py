...
def get_clip_embeddings(frames, device, model, preprocess, batch_size=16):
    ...
    with torch.no_grad():
        for i in range(0, len(frames), batch_size):
            ...
            # Convert to CPU numpy for compatibility with sklearn/numpy operations
...
def maxinfo_frame_selection(frames, R=8, tol=0.3, max_n=64, normalize_svd_output=None):
    ...
    #     indices_uniform = np.linspace(0, len(all_indices) - 1, num=max_n, dtype=int)
    #     all_indices = all_indices[indices_uniform]
