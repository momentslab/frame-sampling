#!/usr/bin/env python3
"""
Model FPS Benchmark Script.

This script benchmarks any supported model with different FPS configurations
to analyze the impact of frame sampling rate on performance and accuracy.
Supports models: ovis, smolvlm, qwen2, qwen2_5, intern
"""
...
def setup_model_arguments():
    """Parse and validate command-line arguments for model FPS benchmark."""
    parser = argparse.ArgumentParser(description="Run model FPS benchmark with different models.")
    parser.add_argument("--model", type=str, required=True,
                       choices=["ovis", "smolvlm", "qwen2", "qwen2_5", "intern"],
                       help="Model to benchmark (ovis, smolvlm, qwen2, qwen2_5, intern)")
    ...
    parser.add_argument("--output_path", type=str, default=None,
                       help="Path to the output (default: ./Asharq_data/model_fps_benchmark/{model}/)")
    ...
    parser.add_argument("--fps_configs", type=str, default="0.1,0.2,0.5,1,1.5,2,2.5,3",
                       help="Comma-separated list of FPS values to test (default: 0.1,0.2,0.5,1,1.5,2,2.5,3)")
    ...
    if args.output_path is None:
        args.output_path = f"./Asharq_data/model_fps_benchmark/{args.model}/"
    else:
        args.output_path = os.path.join(args.output_path, args.model)

    create_output_directory(args.output_path)
    log_arguments(args, f"{args.model.upper()} FPS Benchmark Arguments")
    return args


def load_model_benchmark_data(args):
    """Load and prepare benchmark data for model FPS testing."""
...
def main():
    """Main model FPS benchmark function."""
    set_seed(42)

    args = setup_model_arguments()

    logger = setup_logging(args.output_path, f"{args.model}_fps_benchmark.log")
    logger.info(f"Starting {args.model.upper()} FPS benchmark")

    data_config = load_model_benchmark_data(args)

    fps_list = data_config['fps_list']
    metrics = data_config['metrics'] + ['inference_time']  # Add timing metric
    results_df = pd.DataFrame(index=[f"{args.model}_fps_{str(fps).replace('.', '_')}" for fps in fps_list],
                             columns=metrics)

    logger.info(f"Benchmarking {args.model.upper()} with {len(fps_list)} FPS configurations on {len(data_config['video_paths'])} videos")
    ...
    for fps_value in fps_list:
        ...
        try:
            ...
            scores = process_model(args.model, data_config, args, video_config, f"fps_{fps_str}")

            if scores:
                scores['fps'] = fps_value
                fps_key = f"{args.model}_fps_{fps_str}"
    ...
    final_results_path = os.path.join(args.output_path, f"{args.model}_fps_final_results.csv")
    ...
    fig_path = os.path.join(args.output_path, f"{args.model}_fps_comparison.png")
    plt_fig(plot_df, fig_path)
    logger.info(f"FPS comparison visualization saved to {fig_path}")

    logger.info(f"{args.model.upper()} FPS benchmark completed successfully!")
